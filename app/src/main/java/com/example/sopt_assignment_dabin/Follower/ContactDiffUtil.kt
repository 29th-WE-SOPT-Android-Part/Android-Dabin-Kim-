...
class ContactDiffUtil(private val oldList: MutableList<FollowerResponseDataBio2>, private val currentList: MutableList<FollowerResponseDataBio2>) :
    DiffUtil.Callback() {
...
class ContactDiffUtilBio(private val oldList: List<FollowerResponseDataBio>, private val currentList: List<FollowerResponseDataBio>) :
    ...
    // 각 아이템의 고유 ID값(DB에서 key 같은 것을 받아온다면 그걸 활용)이 있을 것입니다. 이를 활용하여 비교
    override fun areItemsTheSame(oldItemPosition: Int, newItemPosition: Int): Boolean {
        return oldList[oldItemPosition].followerBio == currentList[newItemPosition].followerBio
    }
